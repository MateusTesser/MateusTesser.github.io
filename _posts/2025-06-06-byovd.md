...
A driver needs to runs in kernel space because he has a features or security solutions to needs direct access to hardware, more performance, memory management, directly access to network connection, less abstraction to be effective. 

And it giving it a higher level of freedom—for example, EDR's driver can identify any process as malicious, Bad USBs attacks, directly access to network inspection and so on.

On the other way, why driver runs in kernel mode, is to prevent security solutions from being bypassed by high privilege users.

Why this can be a problem ? Its can be a problem because equals to any other binary, driver has flaws, and flaws can be exploited that what i will show here.
...
These restrictions exist because many APTs use drivers to compromise companies, wipe data, hide malware.
...
Rootkits always be a problem, so microsoft created an program called Windows Hardware Quality Labs to any third-party driver can be submitted his program of driver analysis but, a few malicious drivers been accidentally aproved...Microsoft analysis is not always accurate.

## How does a driver work?

We talked about driver but how its really works ? A driver uses the [IRP (I/O Request Packet) structure](https://learn.microsoft.com/en-us/windows-hardware/drivers/ddi/wdm/ns-wdm-_irp) to manage data within the driver.

Drivers always have a symbolic link. Symbolic Link is a Named Device Object to allow client applications to communicate with the driver.
...
**AssociatedIrp.SystemBuffer** points to a buffer of data submitted by the user for IOCTL functions.
...
First, choose the driver sample you want to reverse engineer. I use [LOLDrivers](https://www.loldrivers.io/) and decided to exploit `gdrv.sys` (Gigabyte driver) but you can choose others like `RTCore64.sys` or `cpuz.sys`.

The entry point of a drive is the `DriverEntry` which is a function exported:
...
Going down you will see a switch function to what IOCTL do you wanna to use

![IOCTL](/assets/byovd/ioctl1.png)

Into the switch he will compare to few IOCTL code and 0xC3502808 IOCTL Code has a memcpy-like function and vulnerable to exploit, lets analyze that!
...
You can load in-memory PDB from the internet, similar to [EDRSandblast](https://raw.githubusercontent.com/wavestone-cdt/EDRSandblast/refs/heads/master/Offsets/ExtractOffsets.py) or calculate from yourself, using patterns or debbugger too.
...
After calculate the offset of a DSE, you need to communicate with a Driver and exploit using the IOCTL found, we already have a symbolic link and MajorFunction
...
And the code to exploit is that, not a monstruosity code and try to avoids KeBugCheck and loads our rootkit.
