...
- **Kernel Patch Guard (KPP)**: Monitors kernel integrity and triggers a Bug Check (BSOD) when corruption or critical function modifications are detected. Notably, KPP's monitoring is not continuous, creating potential exploitation windows.
...
This is where the driver registers its IRP dispatch routines, particularly for handling IOCTL requests.
